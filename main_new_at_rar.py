    ...
    for i, t in enumerate(ids):
        ...
        for ep in prog_bar:
            ...
            for bi in range(0, len(idx), args.batch_size):
                ...
                loss_new += metadata['loss_new'] * len(v_x)
                loss_old += metadata['loss_old'] * len(v_x)
                loss_all += metadata['loss_all'] * len(v_x)
                loss_rar += metadata['loss_rar'] * len(v_x)
                loss_init += metadata['loss_theta^'] * len(v_x)

            loss_new /= len(xtrain)
            loss_old /= len(xtrain)
            loss_all /= len(xtrain)
            loss_rar /= len(xtrain)
            loss_init /= len(xtrain)
            ...
            if args.earlystop:
                ...
                prog_bar.set_description(f'Task: {i} | Ep:{ep+1}/{model.n_epochs} | loss_new: {loss_new :.3f}, loss_old: {loss_old :.3f}, loss_all: {loss_all :.3f}, loss_rar: {loss_rar :.3f}, loss_init: {loss_init :.3f} | val_acc: {100 * valid_acc :.3f}')
...
def life_experience_loader(model, inc_loader, args):
    ...
    for i in range(inc_loader.n_tasks):
        ...
        for ep in range(args.n_epochs):
            ...
            loss_rar /= len(train_loader.dataset)
            loss_init /= len(train_loader.dataset)
            ...
            if args.earlystop:
                ...
                print(f'Task: {task_info["task"]} | Ep:{ep+1}/{model.n_epochs} | loss_new: {loss_new :.3f}, loss_old: {loss_old :.3f}, loss_all: {loss_all :.3f}, loss_rar: {loss_rar :.3f}, loss_init: {loss_init :.3f} | val_acc: {100 * val_acc[-1] :.3f}')
