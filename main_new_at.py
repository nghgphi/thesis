    ...
    for i, t in enumerate(ids):
        ...
        for ep in prog_bar:
            ...
            for bi in range(0, len(idx), args.batch_size):
                ...
                loss_new += metadata['loss_theta^'] * len(v_x)
                # loss_at_new += metadata['loss_at_new'] * len(v_x)
                # loss_old += metadata['loss_old'] * len(v_x)
                # loss_at_old += metadata['loss_at_old'] * len(v_x)
                # grad_penalty += metadata['gradient_penalty'] * len(v_x)
            loss_new /= len(xtrain)
            loss_at_new /= len(xtrain)
            loss_old /= len(xtrain)
            loss_at_old /= len(xtrain)
            grad_penalty /= len(xtrain)
            ...
            if args.earlystop:
                ...
                prog_bar.set_description(f'Task: {i} | Ep:{ep+1}/{model.n_epochs} | loss_new: {loss_new :.3f}, loss_old: {loss_old :.3f}, loss_at_new: {loss_at_new :.3f}, loss_at_old: {loss_at_old :.3f}, grad_penalty: {grad_penalty :.3f} | val_acc: {100 * valid_acc :.3f}')
    ...
    for i in range(inc_loader.n_tasks):
        ...
        for ep in range(args.n_epochs):
            ...
            for (k, (v_x, v_y)) in enumerate(prog_bar):
                ...
                # loss_at_old += metadata['loss_at_old'] * len(v_x)
                # grad_penalty += metadata['gradient_penalty'] * len(v_x)
            loss_new /= len(train_loader.dataset)
            loss_at_new /= len(train_loader.dataset)
            loss_old /= len(train_loader.dataset)
            loss_at_old /= len(train_loader.dataset)
            grad_penalty /= len(train_loader.dataset)
            ...
            if args.earlystop:
                ...
                # print(
                #     "Task: {} | Epoch: {}/{} | time={:2.2f}s | Train: loss={:.3f} loss_at={:.3f} | Valid: loss={:.3f}, acc={:5.1f}% |".format(
                #         task_info['task'], ep + 1, model.n_epochs, time.time() - clock0,
                #         round(train_loss.item(), 5), train_loss_at, round(valid_loss, 5), 100 * val_acc[-1])
                # )
                print(f'Task: {task_info["task"]} | Ep:{ep+1}/{model.n_epochs} | loss_new: {loss_new :.3f}, loss_old: {loss_old :.3f}, loss_at_new: {loss_at_new :.3f}, loss_at_old: {loss_at_old :.3f}, grad_penalty: {grad_penalty :.3f} | val_acc: {100 * val_acc[-1] :.3f}')

                # print(f"Task: {task_info['task']} | Epoch: {ep+1}/{model.n_epochs} time: {time.time() - clock0} | train_loss: {round(train_loss.item(), 5)}, train_loss_at: {round(train_loss_at, 5)} | val_acc: {100 * val_acc[-1]}")
            else:
                print(
                    "Task: {} | Epoch: {}/{} | time={:2.2f}s | Train: loss={:.3f} |".format(task_info['task'],
                        ep + 1, model.n_epochs, model.iter, time.time() - clock0, round(loss_new.item(), 5))
                )
