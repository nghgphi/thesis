...
def life_experience(model, data, ids, args):
    ...
    # model_2 = Model.Net(input_size, n_outputs, n_tasks, args)
    ...
    for i, t in enumerate(ids):
        ...
        for ep in prog_bar:
            ...
            for bi in range(0, len(idx), args.batch_size):
                ...
                if metadata is not None:
                    train_loss += metadata['loss'] * len(v_x)
                    # grad_cur_task += metadata['cur_grad'] * len(v_x)
                    # grad_prev_task += metadata['prev_grad'] * len(v_x)


            train_loss = train_loss / len(xtrain)
            # grad_cur_task = grad_cur_task / len(xtrain)
            # grad_prev_task = grad_prev_task / len(xtrain)
        ...
        for ep in prog_bar:
            ...
            if args.earlystop:
                ...
                prog_bar.set_description_str("Task: {} | Epoch: {}/{} | time={:2.2f}s | Train: loss={:.3f}|".format(
                        i, ep + 1, model.n_epochs, time.time() - clock0, round(train_loss.item(), 5))
                    )
            # model.scheduler.step()
        ...
        # Update Memory of Featureikijimjfewiorusoaie Space
